...
export function KursleiterVerwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: KursleiterVerwaltungDialogProps) {
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "nachname": string | null, // Nachname\n  "email": string | null, // E-Mail\n  "telefon": string | null, // Telefon\n  "qualifikationen": string | null, // Qualifikationen / Zertifizierungen\n  "vorname": string | null, // Vorname\n  "spezialisierungen": LookupValue[] | null, // Spezialisierungen (select one or more keys: "vinyasa" | "yin" | "ashtanga" | "kundalini" | "restorative" | "power" | "prenatal" | "hatha") mapping: vinyasa=Vinyasa Yoga, yin=Yin Yoga, ashtanga=Ashtanga Yoga, kundalini=Kundalini Yoga, restorative=Restorative Yoga, power=Power Yoga, prenatal=Prenatal Yoga, hatha=Hatha Yoga\n  "biografie": string | null, // Biografie\n}`;
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="qualifikationen">Qualifikationen / Zertifizierungen</Label>
            <Textarea
              id="qualifikationen"
          ...
          <div className="space-y-2">
            <Label htmlFor="spezialisierungen">Spezialisierungen</Label>
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Checkbox
                  id="spezialisierungen_prenatal"
                  checked={lookupKeys(fields.spezialisierungen).includes('prenatal')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.spezialisierungen);
                      const next = checked ? [...current, 'prenatal'] : current.filter(k => k !== 'prenatal');
                      return { ...f, spezialisierungen: next.length ? next as any : undefined };
                    });
                  }}
                />
                <Label htmlFor="spezialisierungen_prenatal" className="font-normal">Prenatal Yoga</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
