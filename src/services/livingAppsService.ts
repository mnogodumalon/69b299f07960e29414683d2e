...
export class LivingAppsService {
  ...
  static async getYogaKursManagement(): Promise<YogaKursManagement[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.YOGA_KURS_MANAGEMENT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as YogaKursManagement[];
    return enrichLookupFields(records, 'yoga_kurs_management');
  }
  static async getYogaKursManagementEntry(id: string): Promise<YogaKursManagement | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.YOGA_KURS_MANAGEMENT}/records/${id}`);
    const record = { record_id: data.id, ...data } as YogaKursManagement;
    return enrichLookupFields([record], 'yoga_kurs_management')[0];
  }
  static async createYogaKursManagementEntry(fields: YogaKursManagement['fields']) {
    return callApi('POST', `/apps/${APP_IDS.YOGA_KURS_MANAGEMENT}/records`, { fields });
  }
  static async updateYogaKursManagementEntry(id: string, fields: Partial<YogaKursManagement['fields']>) {
    return callApi('PATCH', `/apps/${APP_IDS.YOGA_KURS_MANAGEMENT}/records/${id}`, { fields });
  }
  static async deleteYogaKursManagementEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.YOGA_KURS_MANAGEMENT}/records/${id}`);
  }
